...
class ECSHelper::Command::ExportEnvSecrets < ECSHelper::Command::Base
  ...
  def export_values
    variables = (['export'] + options[:env_vars].map do |var_name|
      param_name = to_aws_ssm_name(var_name)
      aws_ssm_param = client.get_ssm_parameter(name: param_name, with_decryption: true)
      p aws_ssm_param
      next if aws_ssm_param.empty? || aws_ssm_param.value.empty?
